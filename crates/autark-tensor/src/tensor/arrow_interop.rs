                ...
                ('i', 4) => {
                    ...
                    for col in 0..rows {
                        ...
                        if is_string[col] {
                            ...
                            out.push(Arc::new(Int32Array::from(slice.to_vec())) as ArrayRef);
                        }
                    }
                }
                ('q', 8) => {
                    let vals: Vec<i64> = parse::<8, _>(&raw, i64::from_le_bytes);
                    for col in 0..rows {
                        let start = col * cols;
                        let end = start + cols;
                        let slice = &vals[start..end];
                        if is_string[col] {
                            ...
                            out.push(Arc::new(Int64Array::from(slice.to_vec())) as ArrayRef);
                        }
                    }
                }
                _ => {
